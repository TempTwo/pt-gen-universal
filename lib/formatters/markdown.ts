...
export class MarkdownFormatter implements Formatter {
    ...
    private formatMovie(data: MediaInfo): string {
        ...
        if (imdb_rating || douban_rating || tmdb_rating || bangumi_rating) {
            descr += `\n## 评分\n\n`;
            if (imdb_rating) {
    ...
    private formatGame(data: MediaInfo): string {
        ...
        // Screenshots
        const screenshots = data.screenshots || extra.screenshots || (data.extra?.info_map && data.extra.info_map['screenshots']);
